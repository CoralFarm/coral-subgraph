...
import { Mint, Burn, Swap, Transfer, Sync } from '../types/templates/CRLPair/CRLPair'
import {
    updatePairDayData,
    updateTokenDayData,
    updatePairHourData,
...
export function handleTransfer(event: Transfer): void {
    ...
    let factory = CRLFactory.load(FACTORY_ADDRESS)
    ...
    let pair = CRLPair.load(event.address.toHexString())
...
export function handleSync(event: Sync): void {
    let pair = CRLPair.load(event.address.toHex())
    let token0 = Token.load(pair.token0)
    let token1 = Token.load(pair.token1)
    let crl = CRLFactory.load(FACTORY_ADDRESS)

    // reset factory liquidity by subtracting onluy tarcked liquidity
    crl.totalLiquidityBNB = crl.totalLiquidityBNB.minus(pair.trackedReserveBNB as BigDecimal)
    ...
    crl.totalLiquidityBNB = crl.totalLiquidityBNB.plus(trackedLiquidityBNB)
    crl.totalLiquidityUSD = crl.totalLiquidityBNB.times(bundle.bnbPrice)
...
export function handleMint(event: Mint): void {
    ...
    crl.txCount = crl.txCount.plus(ONE_BI)
...
export function handleSwap(event: Swap): void {
    ...
    let crl = CRLFactory.load(FACTORY_ADDRESS)
    crl.totalVolumeUSD = crl.totalVolumeUSD.plus(trackedAmountUSD)
    crl.totalVolumeBNB = crl.totalVolumeBNB.plus(trackedAmountBNB)
    crl.untrackedVolumeUSD = crl.untrackedVolumeUSD.plus(derivedAmountUSD)
    crl.txCount = crl.txCount.plus(ONE_BI)
    ...
    let crlDayData = updateCRLDayData(event)
    let token0DayData = updateTokenDayData(token0 as Token, event)
    let token1DayData = updateTokenDayData(token1 as Token, event)

    // swap specific updating
    crlDayData.dailyVolumeUSD = crlDayData.dailyVolumeUSD.plus(trackedAmountUSD)
    crlDayData.dailyVolumeBNB = crlDayData.dailyVolumeBNB.plus(trackedAmountBNB)
    crlDayData.dailyVolumeUntracked = crlDayData.dailyVolumeUntracked.plus(derivedAmountUSD)
    crlDayData.save()
