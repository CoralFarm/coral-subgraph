import { ADDRESS_ZERO, BIG_DECIMAL_ZERO, MINIMUM_USD_THRESHOLD_NEW_PAIRS, WHITELIST } from '../constants'
import { Address, BigDecimal, BigInt, log, store } from '@graphprotocol/graph-ts'
import { Burn, Mint, CRLPair, Swap, Token, Transaction } from '../types/schema'
import {
    Burn as BurnEvent,
    Mint as MintEvent,
    CRLPair as PairContract,
    Swap as SwapEvent,
    Sync as SyncEvent,
    Transfer as TransferEvent,
} from '../types/templates/CRLPair/CRLPair'
import {
...
import { findBNBPerToken, getBNBPrice } from '../pricing'
...
export function onTransfer(event: TransferEvent): void {
    ...
    const pair = CRLPair.load(event.address.toHex())
    ...
    // 3 cases, mints, send first on BNB withdrawls, and burns
...
export function onSync(event: SyncEvent): void {
    ...
    factory.liquidityBNB = factory.liquidityBNB.minus(pair.trackedReserveBNB as BigDecimal)
    ...
    bundle.bnbPrice = getBNBPrice(event.block)
    bundle.save()
    ...
    factory.liquidityBNB = factory.liquidityBNB.plus(trackedLiquidityBNB)
    factory.liquidityUSD = factory.liquidityBNB.times(bundle.bnbPrice)
...
export function onMint(event: MintEvent): void {
    ...
    const bundle = getBundle()
    const amountTotalUSD = token1.derivedBNB
        .times(token1Amount)
...
export function onSwap(event: SwapEvent): void {
    ...
    const derivedAmountBNB = token1.derivedBNB
        .times(amount1Total)
    ...
    factory.volumeBNB = factory.volumeBNB.plus(trackedAmountBNB)
    ...
    dayData.volumeBNB = dayData.volumeBNB.plus(trackedAmountBNB)
    ...
    token0DayData.volumeBNB = token0DayData.volumeBNB.plus(amount0Total.times(token1.derivedBNB as BigDecimal))
    ...
    token1DayData.volumeBNB = token1DayData.volumeBNB.plus(amount1Total.times(token1.derivedBNB as BigDecimal))
